...
readId :: B.ByteString -> B.ByteString
readId bs = B.take len (B.drop 17 bs)
  where len = fromIntegral (B.index bs 0)

readPicture :: B.ByteString -> Picture
readPicture bs
    | ((B.index bs 2) == 2) && ((B.index bs 16) == 24) = RGB24 (B.drop (17+id_len+pal_len+1) bs)
    | ((B.index bs 2) == 2) && ((B.index bs 16) == 32) = RGB32 (B.drop (17+id_len+pal_len+1) bs)
    | otherwise                                        = RGB32 (B.drop (17+id_len+pal_len+1) bs)
  where id_len = fromIntegral (B.index bs 0)
        pal_len = fromIntegral ((B.index bs 5) + (shiftL (B.index bs 6) 8))

writeTGA :: String -> TGAData -> IO ()
writeTGA fileName tgaData
    | sizeCorrect tgaData = B.writeFile fileName $ (header tgaData) `B.append` (body tgaData)
    | otherwise           = print "Incorrect size. Not writing image." >> return ()

sizeCorrect :: TGAData -> Bool
...
hasPalette :: Picture -> Word8
hasPalette (PaletteUncompressed _ _) = 1 :: Word8
hasPalette _ = 0 :: Word8

pixelBits :: Picture -> Word8
pixelBits (RGB24 _) = 24
pixelBits (RGB32 _) = 32
pixelBits _         = 0
...
loadTextureFromTGA path = do
    ...
    case picture tga of
        ...
        _       -> putStrLn "Unsupported image type. Images must be either RGB24 or RGB24."
