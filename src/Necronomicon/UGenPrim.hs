...
-- csin p = d
    -- where
        -- (Double d#) = realToFrac . c_sin . realToFrac $ Double p

taylorSin :: Double# -> Double#
taylorSin x = x *## (a0 +## x2 *## (a1 +## x2 *## (a2 +## x2
...
data UGen = UGenFunc   !(Double# -> UGen)
...
instance UGenComponent Double where
   toUGen !v = UGenScalar (# 0,realToFrac v #)

instance UGenComponent [Double] where
   toUGen !v = UGenList $ map (\n -> UGenScalar (# 0,realToFrac n #)) v
...
reduceUgen :: UGen -> Double# -> (# Double#,Double# #)
reduceUgen !(UGenScalar u) _ = u
reduceUgen !(UGenFunc   u) t = u'
    where
        !(UGenScalar u') = u t

calc0 :: UGen -> Double# -> UGen
...
calc :: ((# Double#,Double# #) -> (# Double#,Double# #)) -> UGen -> Double# -> UGen
...
calc2 :: ((# Double#,Double# #) -> (# Double#,Double# #) -> (# Double#,Double# #)) -> UGen -> UGen -> Double# -> UGen
calc2 func !u1 !u2 t = UGenScalar $ func (reduceUgen u1 t) (reduceUgen u2 t)

calc3 :: ((# Double#,Double# #) -> (# Double#,Double# #) -> (# Double#,Double# #) -> (# Double#,Double# #)) -> UGen -> UGen -> UGen -> Double# -> UGen
calc3 func !u1 !u2 !u3 t = UGenScalar $ func (reduceUgen u1 t) (reduceUgen u2 t) (reduceUgen u3 t)

sin :: UGenComponent a => a -> UGen
sin !freq = UGenFunc $ f
  where
      f !t@(frameTime) = calc sinFunc (toUGen freq) t
          where
              sinFunc !(# fa,fo #) = (# taylorSin (fo *## twoPi *## (frameTime /## sampleRate) +## fa) ,0.0## #)
              twoPi = 6.283185307179586##
              sampleRate = 44100.0##
