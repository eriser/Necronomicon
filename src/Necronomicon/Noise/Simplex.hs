...
import Data.Vector as Vec (Vector, (!), fromList)
import GHC.Float
import Data.Bits
import Data.Word
--------------------------------------------------------------------------------
...
simplex2 x y = 0.507 * ( flerp s n0 n1 )
    where
        ...
        nx0 = gradient fx0 fy0 $! perm ! (ix0' + (fromIntegral (perm ! iy0')))
        nx1 = gradient fx0 fy1 $! perm ! (ix0' + (fromIntegral (perm ! iy1)))
        n0	= flerp t nx0 nx1

        nx0'= gradient fx1 fy0 $! perm ! (ix1 + (fromIntegral (perm ! iy0')))
        nx1'= gradient fx1 fy1 $! perm ! (ix1 + (fromIntegral (perm ! iy1)))
        n1 	= flerp t nx0' nx1'
...
simplex featureSize x y = go featureSize 0.0
    where
        go fs v
            | fs < 2    = v
            | otherwise = go (fs/2) $ simplex2 (fs*x) (fs*y) + v
