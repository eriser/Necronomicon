...
vec4 gaussianBlur( sampler2D tex0, vec2 centreUV, vec2 halfPixelOffset, vec2 pixelOffset )
{
    ...
    colOut.a = texture2D(tex0,centreUV).a;
    return colOut;
}

void main()
{
    vec4  bluredTex = gaussianBlur(tex,uv,vec2(halfOffset,halfOffset),vec2(offset,offset));
    float brightness = (bluredTex.x + bluredTex.y + bluredTex.z);
    fragColor = vec4(color,1.0) * (bluredTex + vec4(brightness,brightness,brightness,0.0));
}
